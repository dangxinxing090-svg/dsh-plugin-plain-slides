	...
	factory: (require) => {
		...
		var styles = {
			insert: function (css) {
				...
				tag.dataset.plugin = "dsh-plugin-brief";
				tag.dataset.pluginCss = "dsh-plugin-brief";
				tag.textContent = css;
		...
		var plugin = (function () {
const CSS = `
.dshbrief-card{margin:2px 0 4px;display:flex;flex-direction:column;border:1px solid var(--dsw-alias-border-l1);border-radius:12px;background:var(--dsw-alias-bg-layer-1);overflow:hidden;font-size:13px;line-height:1.6;color:var(--dsw-alias-label-primary);height:calc(100vh - 250px);height:calc(100dvh - 250px);min-height:280px;box-sizing:border-box}
.dshbrief-head{display:flex;align-items:center;gap:8px;padding:7px 10px 7px 12px;border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);flex:0 0 auto}
.dshbrief-badge{font-size:10px;font-weight:600;letter-spacing:.14em;color:var(--dsw-alias-label-secondary)}
.dshbrief-count{font-size:11px;color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums}
.dshbrief-flag{font-size:11px;color:var(--dsw-alias-brand-primary)}
.dshbrief-grow{flex:1 1 auto}
.dshbrief-icon{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;padding:0;border:1px solid transparent;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer;flex:0 0 auto}
.dshbrief-icon:hover{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l1)}
.dshbrief-icon[data-on="true"]{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-border-l2)}
.dshbrief-icontext{display:inline-flex;align-items:center;gap:5px;height:24px;padding:0 9px;border:1px solid transparent;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer;font:inherit;font-size:12px;line-height:1;white-space:nowrap;flex:0 0 auto}
.dshbrief-icontext:hover{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l1)}
.dshbrief-icontext[data-on="true"]{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-border-l2)}
/* A visible rounded frame, for a control that switches what the card shows rather
   than merely revealing something. Declared after the icontext rules so its
   border wins at equal specificity. */
.dshbrief-outlined{border-color:var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2)}
.dshbrief-outlined:hover{border-color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-bg-base)}
.dshbrief-outlined[data-on="true"]{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-bg-base)}
.dshbrief-body{padding:18px 20px;display:flex;flex-direction:column;gap:8px;flex:1 1 auto;min-height:0;overflow:auto}
.dshbrief-kicker{font-size:10.5px;letter-spacing:.12em;text-transform:uppercase;color:var(--dsw-alias-brand-primary);flex:0 0 auto}
.dshbrief-title{font-size:17px;font-weight:600;line-height:1.45;word-break:break-word;flex:0 0 auto}
.dshbrief-lines{display:flex;flex-direction:column;gap:3px;flex:1 1 auto;min-height:0;overflow:auto}
.dshbrief-line{white-space:pre-wrap;word-break:break-word;color:var(--dsw-alias-label-secondary)}
.dshbrief-html{color:var(--dsw-alias-label-secondary);line-height:1.75;word-break:break-word}
.dshbrief-html p{margin:0 0 8px}
.dshbrief-html p:last-child{margin-bottom:0}
.dshbrief-html ul,.dshbrief-html ol{margin:6px 0;padding-left:20px}
.dshbrief-html li{margin:2px 0}
.dshbrief-html b,.dshbrief-html strong{color:var(--dsw-alias-label-primary);font-weight:600}
.dshbrief-html i,.dshbrief-html em{font-style:italic}
.dshbrief-html code{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:.92em;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);border-radius:4px;padding:1px 4px}
.dshbrief-html pre{margin:8px 0;padding:10px 12px;background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l1);border-radius:8px;overflow:auto}
.dshbrief-html pre code{background:transparent;border:none;padding:0;font-size:.9em}
.dshbrief-html mark{background:transparent;color:var(--dsw-alias-brand-primary);font-weight:600}
.dshbrief-html table{border-collapse:collapse;width:100%;margin:8px 0;font-size:.95em;table-layout:fixed}
.dshbrief-html th,.dshbrief-html td{border:1px solid var(--dsw-alias-border-l1);padding:5px 8px;text-align:left;vertical-align:top;word-break:break-word;overflow-wrap:anywhere}
.dshbrief-html th{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font-weight:600}
.dshbrief-meta{margin-top:auto;padding-top:12px;font-size:11px;color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums;flex:0 0 auto;background:var(--dsw-alias-bg-layer-1)}
.dshbrief-foot{display:flex;align-items:center;gap:6px;padding:5px 8px;border-top:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);flex:0 0 auto}
.dshbrief-dots{display:flex;align-items:center;gap:5px;flex:1 1 auto;flex-wrap:wrap;justify-content:center}
.dshbrief-dot{width:6px;height:6px;border-radius:50%;border:none;padding:0;background:var(--dsw-alias-border-l2);cursor:pointer}
.dshbrief-dot[data-on="true"]{background:var(--dsw-alias-brand-primary);width:16px;border-radius:3px}
.dshbrief-dot[data-ask="true"]{background:var(--dsw-alias-state-warn-primary)}
.dshbrief-opts{display:flex;flex-direction:column;gap:8px;margin-top:10px}
.dshbrief-opt{text-align:left;padding:10px 12px;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-alias-bg-layer-2);cursor:pointer;font:inherit;font-size:13px;color:var(--dsw-alias-label-primary)}
.dshbrief-opt:hover{border-color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-bg-base)}
.dshbrief-opt-desc{font-size:11.5px;color:var(--dsw-alias-label-secondary);margin-top:3px}
.dshbrief-answer{margin-top:12px;display:flex;flex-direction:column;gap:8px;flex:0 0 auto}
.dshbrief-ta{width:100%;min-height:62px;resize:vertical;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font:inherit;font-size:13px;padding:8px 10px;box-sizing:border-box}
.dshbrief-row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}
.dshbrief-btn{padding:7px 12px;border-radius:8px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font-size:12.5px;cursor:pointer;font-family:inherit}
.dshbrief-btn:hover{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary)}
.dshbrief-btn[data-primary="true"]{background:var(--dsw-alias-brand-primary);color:#fff;border-color:transparent}
.dshbrief-note{font-size:11.5px;color:var(--dsw-alias-brand-primary)}
.dshbrief-raw{padding:2px 0 6px;white-space:pre-wrap;word-break:break-word;color:var(--dsw-alias-label-primary);font-size:13px;line-height:1.7}
.dshbrief-rawstats{padding:0 0 10px;margin-bottom:10px;border-bottom:1px dashed var(--dsw-alias-border-l1);color:var(--dsw-alias-label-secondary);font-size:11px;font-variant-numeric:tabular-nums}
.dshbrief-rawbody{padding:16px 20px;flex:1 1 auto;min-height:0;overflow:auto;white-space:pre-wrap;word-break:break-word;color:var(--dsw-alias-label-secondary);font-size:13px;line-height:1.75}
.dshbrief-images{display:flex;flex-wrap:wrap;gap:8px;padding-top:10px;flex:0 0 auto}
.dshbrief-action{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;padding:0;border:1px solid transparent;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer}
.dshbrief-action:hover{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-border-l1)}
.dshbrief-stagewrap{position:fixed;inset:0;pointer-events:auto;z-index:60;display:flex;align-items:center;justify-content:center;padding:28px;background:rgba(0,0,0,.42)}
.dshbrief-panel{width:min(980px,100%);height:min(88vh,900px);display:flex;flex-direction:column;border:1px solid var(--dsw-alias-border-l1);border-radius:16px;background:var(--dsw-alias-bg-overlay);overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.34);font-size:14px;color:var(--dsw-alias-label-primary)}
.dshbrief-panel .dshbrief-card{height:100%;margin:0;border:none;border-radius:0;min-height:0}
.dshbrief-panel .dshbrief-title{font-size:19px}
.dshbrief-panel .dshbrief-html{font-size:15px}
/* The bottom margin tops the composer stack's own gap up to 32px — twice the
   16px the transcript column keeps above the box — so the box stands clear of
   the input below it. */
/* Room to breathe inside the box: the padding sets the title off from the top
   border and the step off from the bottom one, the left value is the text's
   distance from the left border, and the outer gap is the text's distance from
   the close control. The inner gap keeps the two lines apart — without it the
   title's line box and the step's sit flush against each other. */
.dshbrief-working{box-sizing:border-box;width:calc(100% - 2 * var(--dsh-composer-side-clearance, 16px) - 4 * var(--dsh-composer-dock-inset, 8px));max-width:calc(var(--dsh-composer-card-max-width, 952px) - 4 * var(--dsh-composer-dock-inset, 8px));margin:0 auto calc(32px - var(--dsh-composer-stack-gap, 6px));border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-alias-bg-layer-1);padding:11px 10px 11px 15px;display:flex;align-items:center;gap:10px;font-size:13px;line-height:1.5;overflow:hidden}
.dshbrief-working-lines{flex:1 1 auto;min-width:0;display:flex;flex-direction:column;gap:5px}
.dshbrief-working-title{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--dsw-static-deepseek-500, var(--dsw-alias-brand-primary))}
.dshbrief-working-line{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--dsw-alias-label-secondary);min-height:1.5em}
...
[data-chat-flow-kind="assistant-step"]:not(:has(.dshbrief-card)){display:none!important}
`
...
/**
 * Retries allowed after the first rewrite attempt, so a turn costs at most
 * `REWRITE_MAX_RETRIES + 1` model calls and usually exactly one.
 */
const REWRITE_MAX_RETRIES = 3

/**
 * Drive the rewrite to a decision under a hard call budget.
 *
 * One attempt, then at most `REWRITE_MAX_RETRIES` more, and not one call further
 * once a usable deck comes back — a success ends the turn's budget immediately.
 * The budget lives here, and only here: the host half used to keep a retry of
 * its own, which quietly doubled whatever cap this half thought it was applying.
 *
 * `callOnce(attempt)` resolves to the parsed deck, or to `null` for any failure
 * — a rejected route, a refused call, or an answer that yielded no usable deck.
 * `settle(state, slides)` is called exactly once, with `'plain'` and the deck or
 * with `'error'` and `null`.
 */
function runRewrite(callOnce, settle) {
  let attempt = 0

  function gaveUp() {
    return attempt >= REWRITE_MAX_RETRIES
  }

  function retryOrSettle(why) {
    if (gaveUp()) {
      settle('error', null, why)
      return
    }
    attempt += 1
    step()
  ...
  function step() {
    callOnce(attempt).then(function (slides) {
      if (slides && slides.length) {
        settle('plain', slides)
        return
      }
      retryOrSettle('no usable deck')
    }, function (error) {
      retryOrSettle(error)
    })
  }
...
/**
 * Serialise the rewrites: at most one in flight, the rest waiting. A reload
 * re-renders every turn in the loaded window at once, and without this they
 * would all ask the model together — the burst this plugin must not create.
 *
 * A kept deck never reaches the queue, so in steady state the queue is empty and
 * only the turn that just finished ever travels through it.
 */
function createRewriteQueue() {
  let busy = false
  const waiting = []
...
/**
 * Only the closing step of a settled turn is rewritten.
 *
 * Everything before it is the process: this plugin never renders those steps and
 * must never pay a model call for them. Returns the text to rewrite, or null
 * when this step is not the turn's report.
 */
function rewriteTarget(settled, isClosing, text) {
  if (!settled || !isClosing) return null
  if (typeof text !== 'string' || text.length === 0) return null
  return text
}

/**
 * Where a successful rewrite is kept so that reopening DSH shows the plain
 * version without asking the model again. A rewrite is a pure function of the
 * closing step's text, and that text never changes once the turn has closed, so
 * one success per turn is enough forever.
 *
 * Bump the version prefix when the rewrite prompt or protocol changes: stale
 * decks then stop being read instead of being shown against a new protocol.
 */
const PLAIN_STORE_KEY = 'dshbrief:plain-decks'
const PLAIN_STORE_VERSION = 'v1'
const PLAIN_STORE_MAX = 120

/**
 * The rename to `brief` moved the storage namespace from `dshdeck` to `dshbrief`.
 * Move whatever is under the old keys once, so a reader's preference and the
 * decks they already paid for survive the rename instead of silently resetting.
 *
 * This is the only place the old namespace is still named, and it can be deleted
 * one release after the rename ships.
 */
function migrateLegacyKeys(pairs) {
  try {
    ...
    // A blocked store costs the migration, never the plugin.
  }
}

function plainStoreKey(sessionId, turnNo) {
  const session = sessionId === undefined || sessionId === null ? '' : String(sessionId)
  return PLAIN_STORE_VERSION + ':' + session + ':' + turnNo
}

function readPlainStore() {
  try {
    if (typeof window === 'undefined' || !window.localStorage) return {}
    const raw = window.localStorage.getItem(PLAIN_STORE_KEY)
    if (typeof raw !== 'string' || raw === '') return {}
    const parsed = JSON.parse(raw)
    // An array passes `typeof === 'object'` but cannot hold string keys through
    // JSON, so a store that somehow became an array must read as empty rather
    // than silently accepting writes that never persist.
...
/** The kept deck for one turn, or null. Never throws. */
function readPlainDeck(sessionId, turnNo) {
  try {
    const entry = readPlainStore()[plainStoreKey(sessionId, turnNo)]
    return entry && Array.isArray(entry.slides) && entry.slides.length ? entry.slides : null
  } catch (error) {
    return null
  }
}
...
/**
 * What one turn's rewrite should do: reuse a deck that was already paid for, or
 * call the model for one. This is the whole "a success is enough forever" rule,
 * in one place, so it can be asserted rather than inferred.
 */
function rewritePlan(sessionId, turnNo) {
  const kept = readPlainDeck(sessionId, turnNo)
  return kept === null ? { state: 'call' } : { state: 'reuse', slides: kept }
}

/**
 * Keep one successful deck, evicting the least recently written when the store
 * is full. A blocked or full store costs the cache, never the deck.
 */
function writePlainDeck(sessionId, turnNo, slides) {
  try {
    if (typeof window === 'undefined' || !window.localStorage) return
    if (!Array.isArray(slides) || !slides.length) return
    const store = readPlainStore()
    store[plainStoreKey(sessionId, turnNo)] = { slides: slides, at: Date.now() }
    const keys = Object.keys(store)
    if (keys.length > PLAIN_STORE_MAX) {
      keys.sort(function (a, b) {
        return (store[a] && store[a].at ? store[a].at : 0) - (store[b] && store[b].at ? store[b].at : 0)
      })
      for (let i = 0; i < keys.length - PLAIN_STORE_MAX; i++) delete store[keys[i]]
    }
    window.localStorage.setItem(PLAIN_STORE_KEY, JSON.stringify(store))
  } catch (error) {
    // Ignore: the store is an optimisation, not a requirement.
  }
}
...
/**
 * The working box is exactly two lines: this title, then one process line. The
 * title also tells the reader where the detail actually lives, which is the
 * whole point of not putting the process in the transcript.
 */
const WORKING_TITLE = '正在进行以下专业编程操作，想了解过程细节，请到轨迹页查看'

/** How many process lines the working box keeps on screen, under the title. */
...
function QuestionSlide(props) {
  ...
  if (options.length) {
    children.push(React.createElement(
      'div',
      { key: 'o', className: 'dshbrief-opts' },
      options.map(function (opt, i) {
        return React.createElement(
          'button',
          { key: i, type: 'button', className: 'dshbrief-opt', onClick: function () { fill(opt.label) } },
          opt.label,
          opt.desc ? React.createElement('div', { className: 'dshbrief-opt-desc' }, opt.desc) : null,
        )
      }),
    ))
  }
  children.push(React.createElement(
    'div',
    { key: 'a', className: 'dshbrief-answer' },
    React.createElement('textarea', {
      className: 'dshbrief-ta',
      value: custom,
    ...
    React.createElement(
      'div',
      { className: 'dshbrief-row' },
      React.createElement('button', { type: 'button', className: 'dshbrief-btn', onClick: function () { fill(custom) } }, '\u586b\u5165\u8f93\u5165\u6846'),
      React.createElement('button', { type: 'button', className: 'dshbrief-btn', 'data-primary': 'true', onClick: send }, '\u53d1\u9001'),
      filled ? React.createElement('span', { className: 'dshbrief-note' }, '\u5df2\u586b\u5165\uff1a' + truncate(filled, 40)) : null,
    ),
  ))
  return React.createElement('div', { className: 'dshbrief-body' }, children)
}
...
function Deck(props) {
  ...
  // Rewriting is opt-in. The original deck is always built locally and is what
  // the card opens on; the plain one exists only once the reader has asked for
  // it. Both are on hand afterwards, so the card can move between the two
  // wordings without leaving the deck and without another model call.
  const plainSlides = Array.isArray(props.plainSlides) && props.plainSlides.length ? props.plainSlides : null
  const [wantsPlain, setWantsPlain] = React.useState(false)
  const showingPlain = wantsPlain && plainSlides !== null
  const slides = showingPlain ? plainSlides : props.slides
  ...
  const cardHeight = convHeight === null ? null : Math.max(300, Math.min(600, Math.round(convHeight * 0.6)))
  ...
  const canSwitch = plainSlides !== null
  const pending = plainState === 'pending'
  const canAsk = !canSwitch && !pending && typeof props.onRequestPlain === 'function'
  const flag = pending
    ? '\u901a\u4fd7\u5316\u4e2d\u2026'
    : rawMode
      ? '\u539f\u6587'
  ...
  const head = React.createElement(
    'div',
    { className: 'dshbrief-head' },
    React.createElement('span', { className: 'dshbrief-badge' }, variant === 'stage' ? 'DECK' : 'SLIDES'),
    React.createElement('span', { className: 'dshbrief-count' }, index + 1 + ' / ' + total),
    flag ? React.createElement('span', { className: 'dshbrief-flag' }, '\u00b7 ' + flag) : null,
    (canSwitch || canAsk) && !rawMode
      ? React.createElement(
          'button',
          {
            type: 'button',
            className: 'dshbrief-icontext dshbrief-outlined',
            'data-on': showingPlain ? 'true' : 'false',
            'aria-label': showingPlain ? '\u67e5\u770b\u539f\u7248' : '\u628a\u8fd9\u4e00\u8f6e\u901a\u4fd7\u5316',
            title: showingPlain ? '\u67e5\u770b\u539f\u7248' : '\u628a\u8fd9\u4e00\u8f6e\u901a\u4fd7\u5316',
            onClick: function () {
              if (canSwitch) {
                setWantsPlain(function (v) { return !v })
                setPage(0)
                return
              }
              setWantsPlain(true)
              props.onRequestPlain()
            },
          },
          showingPlain ? '\u539f\u7248' : '\u901a\u4fd7\u5316',
        )
      : null,
    React.createElement('span', { className: 'dshbrief-grow' }),
    fullText
      ? React.createElement(
          'button',
          {
            type: 'button',
            className: 'dshbrief-icontext',
          ...
          textIcon(),
          React.createElement('span', null, rawMode ? '\u56de\u5230\u5e7b\u706f\u7247' : '\u5b8c\u6574\u539f\u6587'),
        )
      : null,
    props.onClose
      ? React.createElement('button', { type: 'button', className: 'dshbrief-icon', 'aria-label': '\u5173\u95ed', title: '\u5173\u95ed', onClick: props.onClose }, closeIcon())
      : null,
  )
  ...
  if (rawMode) {
    const kids = []
    if (technicalText) kids.push(React.createElement('div', { key: 's', className: 'dshbrief-rawstats' }, technicalText))
    kids.push(React.createElement('div', { key: 't', className: 'dshbrief-raw' }, fullText))
    body = React.createElement('div', { className: 'dshbrief-rawbody' }, kids)
    ...
    const linesNode = typeof slide.html === 'string'
      ? React.createElement('div', { className: 'dshbrief-html', dangerouslySetInnerHTML: { __html: slide.html } })
      : (slide.lines || []).map(function (line, i) {
          return React.createElement('div', { key: i, className: 'dshbrief-line' }, line === '' ? '\u00a0' : line)
        })
    body = React.createElement(
      'div',
      { className: 'dshbrief-body' },
      React.createElement('div', { className: 'dshbrief-kicker' }, slide.kicker || ''),
      React.createElement('div', { className: 'dshbrief-title' }, slide.title || ''),
      React.createElement('div', { className: 'dshbrief-lines' }, linesNode),
      slide.meta ? React.createElement('div', { className: 'dshbrief-meta' }, slide.meta) : null,
      imageNode ? React.createElement('div', { className: 'dshbrief-images' }, imageNode) : null,
    )
  }

  const foot = React.createElement(
    'div',
    { className: 'dshbrief-foot' },
    React.createElement('button', { type: 'button', className: 'dshbrief-icon', 'aria-label': '\u4e0a\u4e00\u9875', title: '\u4e0a\u4e00\u9875', onClick: function () { go(-1) } }, chevron(-1)),
    React.createElement(
      'div',
      { className: 'dshbrief-dots' },
      slides.map(function (s, i) {
        return React.createElement('button', {
          key: i, type: 'button', className: 'dshbrief-dot',
    ...
    React.createElement('button', { type: 'button', className: 'dshbrief-icon', 'aria-label': '\u4e0b\u4e00\u9875', title: '\u4e0b\u4e00\u9875', onClick: function () { go(1) } }, chevron(1)),
  )

  return React.createElement(
    'div',
    {
      className: 'dshbrief-card dshbrief-' + variant,
      ref: rootRef,
...
__expose({ buildSlides: buildSlides, slidesFromPlain: slidesFromPlain, plainModelView: plainModelView, runRewrite: runRewrite, REWRITE_MAX_RETRIES: REWRITE_MAX_RETRIES, rewriteTarget: rewriteTarget, rewritePlan: rewritePlan, createRewriteQueue: createRewriteQueue, readPlainDeck: readPlainDeck, writePlainDeck: writePlainDeck, plainStoreKey: plainStoreKey, PLAIN_STORE_KEY: PLAIN_STORE_KEY, PLAIN_STORE_MAX: PLAIN_STORE_MAX, markdownToHtml: markdownToHtml, sanitizeHtml: sanitizeHtml, toolLabel: toolLabel, TOOL_LABELS: TOOL_LABELS, workingLines: workingLines, runningTurnNo: runningTurnNo, targetHint: targetHint, baseName: baseName })
return {
  apply(ctx) {
    const slots = ctx.get('slots')
    if (slots === undefined) return

    // Before anything reads a key: carry the pre-rename namespace across.
    migrateLegacyKeys([
      ['dshdeck:working-box-closed', 'dshbrief:working-box-closed'],
      ['dshdeck:plain-decks', 'dshbrief:plain-decks'],
    ])

    ctx.effect(function () { return styles.insert(CSS) }, 'dshbrief: styles')

    const plainCache = new Map()
    const plainSubs = new Set()
    const plainModelStash = new Map()
    const rewriteQueue = createRewriteQueue()
    ...
    const WORKING_BOX_KEY = 'dshbrief:working-box-closed'
    ...
    /**
     * One entry per turn, created before the first call goes out and never
     * replaced: a turn that has already asked — successfully or not — never asks
     * again, so a re-render cannot spend the budget twice.
     *
     * A deck kept from an earlier session is not an ask at all: it is the answer,
     * and the budget is never opened for it.
     */
    function plainEntryFor(turnNo, text, sessionId) {
      const key = 'p' + turnNo
      const existing = plainCache.get(key)
      if (existing !== undefined) return existing

      const plan = rewritePlan(sessionId, turnNo)
      if (plan.state === 'reuse') {
        const reused = { state: 'plain', slides: plan.slides, attempts: 0, reused: true }
        plainCache.set(key, reused)
        notifyPlain()
        return reused
      }

      const entry = { state: 'pending', slides: null, attempts: 0 }
      plainCache.set(key, entry)

      function callOnce(attempt) {
        entry.attempts = attempt + 1
        return host.call('plain', { turn: turnNo, text: text, attempt: attempt }).then(
          function (res) {
            const stash = plainModelStash.get(key)
            const view = stash === undefined ? plainModelView(null) : stash
            return res && res.ok === true && typeof res.text === 'string' ? slidesFromPlain(res.text, view) : null
          },
          function () {
            return null
          },
      ...
      rewriteQueue.push(function (done) {
        runRewrite(callOnce, function (state, slides, why) {
          entry.state = state
          entry.slides = slides
          if (state === 'plain') {
            // One success is kept forever; the turn never asks again, in this
            // session or the next.
            writePlainDeck(sessionId, turnNo, slides)
          } else if (typeof console !== 'undefined' && typeof console.warn === 'function') {
            // Never swallow this silently. A rewrite that fails and falls back is
            // normal; one that fails on every turn because the plumbing is wrong is
            // a bug, and it stayed hidden for the life of the feature because the
            // failure path said nothing.
            console.warn(
              'brief: the rewrite gave up after ' + entry.attempts + ' attempt(s); the local rendering is shown instead',
              why,
            )
          }
          notifyPlain()
    ...
    /**
     * The rewrite is opt-in, so this returns the turn's entry — null until the
     * reader asks — alongside the ask itself. Nothing is sent on its own: a turn
     * costs no model call unless someone presses the button.
     *
     ...
     * `text` is null when this step is not the turn's report, which also means
     * there is nothing to ask for.
     */
    function usePlainEntry(turnNo, text, model, sessionId) {
      const [, setTick] = React.useState(0)
      React.useEffect(function () {
        const listener = function () { setTick(function (v) { return v + 1 }) }
      ...
      const request = text === null ? null : function () {
        if (plainCache.has('p' + turnNo)) return
        plainModelStash.set('p' + turnNo, plainModelView(model))
        plainEntryFor(turnNo, text, sessionId)
      }
      return { entry: entry === undefined ? null : entry, request: request }
    }
    ...
    function WorkingBox(props) {
      ...
      return React.createElement(
        'div',
        { className: 'dshbrief-working' },
        React.createElement(
          'div',
          { className: 'dshbrief-working-lines' },
          React.createElement('div', { className: 'dshbrief-working-title', title: WORKING_TITLE }, WORKING_TITLE),
          lines.map(function (line, i) {
            return React.createElement(
              'div',
              { key: i, className: 'dshbrief-working-line', title: line },
              line,
            )
          }),
        ),
    ...
    /**
     * The working-box switch in the session header, and the only way back into a
     * box that has been closed — closing is a persisted preference, so without
     * this the recovery would be clearing browser storage.
     *
     * It is always on screen, in both states, with `data-on` showing which one.
     * An earlier version only appeared when a turn was running *and* the box was
     * closed, which made the control invisible for exactly the person looking for
     * it: a hover-less header icon that is missing most of the time reads as a
     * control that does not exist. Living in the header also keeps the promise
     * that a closed box leaves the conversation with nothing but the report.
     */
    function WorkingToggle() {
      const closed = useWorkingBoxClosed()
      // A toggle names what pressing it does, so the label follows the state:
      // `显示工作过程` while the box is closed, `隐藏工作过程` while it is showing.
      const label = closed ? '显示工作过程' : '隐藏工作过程'
      return React.createElement(
        'button',
        {
          type: 'button',
          className: 'dshbrief-icontext',
          'data-on': closed ? 'false' : 'true',
          'aria-pressed': closed ? 'false' : 'true',
          'aria-label': label,
          title: label,
          onClick: function () { setWorkingBoxClosed(!closed) },
        },
        textIcon(),
    ...
    function AssistantStepView(props) {
      ...
      // Only the turn's report is ever rewritten — `rewriteTarget` returns null
      // for a running or mid-turn step, so the process never reaches the model.
      const plainText = turnNo >= 0 ? rewriteTarget(settled, isClosing, text) : null
      const model = buildModel(chat, turnNo)
      const localSlides = buildSlides(model, text)
      const plain = usePlainEntry(turnNo, plainText, model, props.sessionId)
      const plainEntry = plain.entry
      ...
      // The original deck is always built and is what the card opens on. A plain
      // one exists only if the reader asked for it, here or in an earlier
      // session.
      const plainState = plainEntry === null ? 'local' : plainEntry.state
      const plainSlides = plainState === 'plain' && plainEntry.slides && plainEntry.slides.length ? plainEntry.slides : null
      // While a rewrite is in flight the card previews its own conclusion rather
      // than pretending nothing is happening.
      const slides = plainState === 'pending' && localSlides.length ? [localSlides[0]] : localSlides
      ...
      return React.createElement(Deck, {
        model: model,
        slides: slides,
        plainSlides: plainSlides,
        onRequestPlain: plain.request,
        variant: 'inline',
        onClose: null,
        fullText: text,
    ...
    function DeckAction(props) {
      ...
      function onOpen() {
        ...
        const model = buildModel(chat, turnNo)
        const entry = plainCache.get('p' + turnNo)
        const rewritten = entry && entry.state === 'plain' && entry.slides ? entry.slides : null
        const local = buildSlides(model, model.text)
        openStage({
          turn: turnNo,
          model: model,
          slides: local,
          plainSlides: rewritten,
          fullText: model.text,
          technicalText: technicalBits(model),
      ...
      return React.createElement(
        'button',
        { type: 'button', className: 'dshbrief-action', 'aria-label': '\u6f14\u793a\uff1a\u5927\u5c4f\u5e7b\u706f\u7247', title: '\u6f14\u793a\uff1a\u5927\u5c4f\u5e7b\u706f\u7247', onClick: onOpen },
        screenIcon(),
      )
    }
    ...
    function StageOverlay() {
      ...
      return React.createElement(
        'div',
        { className: 'dshbrief-stagewrap', onClick: function (event) { if (event.target === event.currentTarget) closeStage() } },
        React.createElement(
          'div',
          { className: 'dshbrief-panel' },
          React.createElement(Deck, {
            model: stage.model,
            slides: stage.slides,
            plainSlides: stage.plainSlides,
            variant: 'stage',
            onClose: closeStage,
    ...
    slots.inject('conversation.input.dock', function () {
      return slots.register(
        { name: 'conversation.input.dock', id: 'dshbrief-working', order: -10 },
        WorkingBox,
      )
    })

    // The way back to a closed box. The shipped utilities there run at order
    // -10 and 0; a higher order puts this last, at the right edge.
    slots.inject('conversation.session.header.utilities', function () {
      return slots.register(
        { name: 'conversation.session.header.utilities', id: 'dshbrief-working-toggle', order: 30, label: '\u5de5\u4f5c\u8fc7\u7a0b' },
        WorkingToggle,
      )
    })
    ...
    slots.inject('conversation.chat.assistant-actions', function () {
      return slots.register(
        { name: 'conversation.chat.assistant-actions', id: 'dshbrief-stage', order: 20, label: '\u6f14\u793a' },
        DeckAction,
      )
    })

    slots.inject('shell.overlay', function () {
      return slots.register({ name: 'shell.overlay', id: 'dshbrief-stage-overlay', order: 40 }, StageOverlay)
    })
  },
}
