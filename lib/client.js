...
.dshdeck-icontext{display:inline-flex;align-items:center;gap:5px;height:24px;padding:0 9px;border:1px solid transparent;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer;font:inherit;font-size:12px;line-height:1;white-space:nowrap;flex:0 0 auto}
.dshdeck-icontext:hover{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l1)}
.dshdeck-icontext[data-on="true"]{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-border-l2)}
...
.dshdeck-outlined{border-color:var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2)}
.dshdeck-outlined:hover{border-color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-bg-base)}
.dshdeck-outlined[data-on="true"]{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-bg-base)}
...
.dshdeck-working-lines{flex:1 1 auto;min-width:0;display:flex;flex-direction:column;gap:5px}
.dshdeck-working-title{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--dsw-static-deepseek-500, var(--dsw-alias-brand-primary))}
 ...
 */
const PLAIN_STORE_KEY = 'dshdeck:plain-decks'
  ...
  const head = React.createElement(
    ...
    (canSwitch || canAsk) && !rawMode
      ? React.createElement(
          ...
          {
            type: 'button',
            className: 'dshdeck-icontext dshdeck-outlined',
...
return {
  apply(ctx) {
      ...
      rewriteQueue.push(function (done) {
        runRewrite(callOnce, function (state, slides, why) {
          ...
          if (state === 'plain') {
            ...
            console.warn(
              'plain-slides: the rewrite gave up after ' + entry.attempts + ' attempt(s); the local rendering is shown instead',
              why,
            )
          }
          notifyPlain()
    ...
    function WorkingBox(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement('div', { className: 'dshdeck-working-title', title: WORKING_TITLE }, WORKING_TITLE),
          lines.map(function (line, i) {
            return React.createElement(
              'div',
              { key: i, className: 'dshdeck-working-line', title: line },
              line,
            )
          }),
        ),
    ...
    slots.inject('conversation.session.header.utilities', function () {
      return slots.register(
        { name: 'conversation.session.header.utilities', id: 'dshdeck-working-toggle', order: 30, label: '\u5de5\u4f5c\u8fc7\u7a0b' },
        WorkingToggle,
      )
    })
