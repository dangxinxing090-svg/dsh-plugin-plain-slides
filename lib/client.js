...
.dshdeck-waiting{display:flex;align-items:center;gap:9px;padding:10px 2px;color:var(--dsw-alias-label-secondary);font-size:13px}
.dshdeck-waiting-dot{width:7px;height:7px;border-radius:50%;background:var(--dsw-alias-brand-primary);animation:dshdeck-waiting-pulse 1.4s ease-in-out infinite}
@keyframes dshdeck-waiting-pulse{0%,100%{opacity:.2}50%{opacity:1}}
`
...
__expose({ buildSlides: buildSlides, slidesFromPlain: slidesFromPlain, markdownToHtml: markdownToHtml, sanitizeHtml: sanitizeHtml, toolLabel: toolLabel, TOOL_LABELS: TOOL_LABELS })
return {
  apply(ctx) {
    ...
    function plainEntryFor(turnNo, text) {
      ...
      host
        ...
        .then(function (res) {
          ...
          const parsed = res && res.ok === true && typeof res.text === 'string' ? slidesFromPlain(res.text, stash ? stash : { prompt: '' }) : null
...
/** One calm line while a turn is still running; the detail is in the Trajectory. */
    function WaitingView() {
      return React.createElement(
        'div',
        { className: 'dshdeck-waiting' },
        React.createElement('span', { className: 'dshdeck-waiting-dot' }),
        React.createElement('span', null, '正在处理，有结果了第一时间给您汇报'),
      )
    }
    ...
    function AssistantStepView(props) {
      ...
      const plainEntry = usePlainEntry(turnNo, text, usePlain, { prompt: model.prompt })

      // While the turn runs, the transcript would otherwise stream the agent's
      // narration and its tool rows. Show one placeholder on the last assistant
      // step of the turn, and nothing on the earlier ones.
      if (!settled) {
        if (node && lastKey !== null && node.key === lastKey) return React.createElement(WaitingView, null)
        return null
      }
    ...
    // These kinds carry the agent's working process rather than its report: the
    // fold controller over reasoning and tool calls, injected context,
    // housekeeping markers, retry notices and the system
    // prompt card. Rendering them as nothing leaves the transcript as "your
    // messages plus the final deck"; every detail stays in the Trajectory view.
    // Failures (turn-error, turn-max-tokens) and the turn footer are
    // deliberately absent from this list.
    // `tool-call` is deliberately NOT in this list: it owns the
    // `tool.call.toolview` child slot, where every interactive tool card lives,
    // including the exit_plan_mode plan review, which has no other client
    // surface anywhere. A conversation.chat.node renderer receives no
    // renderSlot prop, so a replacement cannot delegate back to those cards.
    // Tool rows are visible only while a turn runs anyway; once it closes they
    // fold into turn-process, which is hidden here.
    ...
    for (let i = 0; i < HIDDEN_NODE_KINDS.length; i++) {
      ...
      slots.inject('conversation.chat.node', function () {
        return slots.register({ name: 'conversation.chat.node', key: hiddenKind }, function () { return null })
      })
    }
