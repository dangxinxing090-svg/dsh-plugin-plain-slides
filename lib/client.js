...
/* The bottom margin tops the composer stack's own gap up to the 16px the
   transcript column keeps above the box, so the box stands as far from the input
   as it does from the conversation. */
...
.dshdeck-working{box-sizing:border-box;width:calc(100% - 2 * var(--dsh-composer-side-clearance, 16px) - 4 * var(--dsh-composer-dock-inset, 8px));max-width:calc(var(--dsh-composer-card-max-width, 952px) - 4 * var(--dsh-composer-dock-inset, 8px));margin:0 auto calc(16px - var(--dsh-composer-stack-gap, 6px));border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-alias-bg-layer-1);padding:11px 10px 11px 15px;display:flex;align-items:center;gap:10px;font-size:13px;line-height:1.5;overflow:hidden}
...
function Deck(props) {
  ...
  // The rewritten deck and the locally rendered one are the same turn in two
  // wordings. When the rewrite succeeded both are on hand, so the reader can
  // move between them without leaving the deck — and without another model call.
  const originalSlides = Array.isArray(props.originalSlides) && props.originalSlides.length ? props.originalSlides : null
  const [showOriginal, setShowOriginal] = React.useState(false)
  const slides = showOriginal && originalSlides !== null ? originalSlides : props.slides
  ...
  const canSwitch = originalSlides !== null
  const flag = plainState === 'pending'
    ? '\u901a\u4fd7\u5316\u4e2d\u2026'
    : rawMode
      ? '\u539f\u6587'
      : canSwitch
        ? (showOriginal ? '\u539f\u7248' : '\u901a\u4fd7\u7248')
        : plainState === 'plain' ? '\u901a\u4fd7\u7248' : ''
    ...
    canSwitch && !rawMode
      ? React.createElement(
          'button',
          {
            ...
            'data-on': showOriginal ? 'true' : 'false',
            'aria-label': showOriginal ? '\u5207\u56de\u901a\u4fd7\u7248' : '\u67e5\u770b\u539f\u7248',
            title: showOriginal ? '\u5207\u56de\u901a\u4fd7\u7248' : '\u67e5\u770b\u539f\u7248',
            onClick: function () {
              setShowOriginal(function (v) { return !v })
              setPage(0)
            },
          },
          showOriginal ? '\u901a\u4fd7\u7248' : '\u539f\u7248',
...
return {
  apply(ctx) {
     ...
     */
    function usePlainEntry(turnNo, text, enabled, model, sessionId) {
      const [, setTick] = React.useState(0)
      React.useEffect(function () {
        const listener = function () { setTick(function (v) { return v + 1 }) }
      ...
      const settledEntry = plainCache.get('p' + turnNo)
      if (settledEntry !== undefined) return settledEntry
      if (!enabled) return null
      // First look at this turn in this mount: a kept deck is already the answer,
      // so seed it here rather than after the effect, and the card never flashes
      // the local rendering on its way to the plain one.
      const plan = rewritePlan(sessionId, turnNo)
      if (plan.state !== 'reuse') return null
      const seeded = { state: 'plain', slides: plan.slides, attempts: 0, reused: true }
      plainCache.set('p' + turnNo, seeded)
      return seeded
    }
    ...
    function AssistantStepView(props) {
      ...
      const plainEntry = usePlainEntry(turnNo, plainText, plainText !== null, model, props.sessionId)
      ...
      const plainState = plainText === null ? 'local' : plainEntry === null ? 'pending' : plainEntry.state
      const rewritten = plainState === 'plain' && plainEntry.slides && plainEntry.slides.length ? plainEntry.slides : null
      let slides = localSlides
      let originalSlides = null
      if (rewritten !== null) {
        // Both wordings are ready, so the card can offer the switch between them.
        slides = rewritten
        originalSlides = localSlides
    ...
    function DeckAction(props) {
      ...
      function onOpen() {
        ...
        openStage({
          turn: turnNo,
          model: model,
          slides: rewritten !== null ? rewritten : local,
          originalSlides: rewritten !== null ? local : null,
          fullText: model.text,
          technicalText: technicalBits(model),
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(Deck, {
            model: stage.model,
            slides: stage.slides,
            originalSlides: stage.originalSlides,
            variant: 'stage',
            onClose: closeStage,
