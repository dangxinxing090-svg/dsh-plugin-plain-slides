...
__expose({ buildSlides: buildSlides, slidesFromPlain: slidesFromPlain, plainModelView: plainModelView, runRewrite: runRewrite, REWRITE_MAX_RETRIES: REWRITE_MAX_RETRIES, markdownToHtml: markdownToHtml, sanitizeHtml: sanitizeHtml, toolLabel: toolLabel, TOOL_LABELS: TOOL_LABELS, workingLines: workingLines, runningTurnNo: runningTurnNo, targetHint: targetHint, baseName: baseName })
return {
  apply(ctx) {
    ...
    function plainEntryFor(turnNo, text) {
      ...
      runRewrite(callOnce, function (state, slides, why) {
        ...
        if (state !== 'plain' && typeof console !== 'undefined' && typeof console.warn === 'function') {
          console.warn(
    ...
    /**
     * The way back in. Closing the box is a persisted preference, so without
     * this the only recovery would be clearing browser storage. It appears in
     * the session header exactly when there is something to reopen — a turn is
     * running and the box is closed — and stays out of the way otherwise, so a
     * closed box still leaves the conversation with nothing but the report.
     */
    function WorkingToggle(props) {
      const closed = useWorkingBoxClosed()
      const useChat = typeof props.useChat === 'function' ? props.useChat : useNothing
      const chat = useChat(function (s) { return s })
      if (!closed || runningTurnNo(chat) < 0) return null
      return React.createElement(
        'button',
        {
          type: 'button',
          ...
          'aria-label': '显示工作过程',
          title: '显示工作过程',
          onClick: function () { setWorkingBoxClosed(false) },
        },
        textIcon(),
      )
    }
