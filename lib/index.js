  ...
  apply(ctx) {
    harness.handle('plain', async function (args) {
      try {
        ...
        // The retry budget is owned by the client, which is the only half that
        // can count calls across the whole turn. This half must therefore issue
        // exactly one model call per request: it used to add a retry of its own,
        // which silently doubled whatever budget the caller thought it had.
        // A repeat attempt from the client is the one that gets the wider cap,
        // because an empty answer is usually a reasoning model spending the
        // whole budget on reasoning and still finishing cleanly.
        const attempt = typeof args.attempt === 'number' && args.attempt > 0 ? args.attempt : 0
        const options = {
          provider: provider,
          model: model,
          ...
          temperature: 0.2,
          maxTokens: attempt > 0 ? 16000 : 8000,
        }
        if (effort !== '') options.reasoningEffort = effort

        const collected = await collectText(llm.stream(options))
